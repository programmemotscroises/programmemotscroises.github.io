...
function SelectObligatoires(txt) {
  ...
  divresult.innerHTML +=
    '<div class="Obligatoire ask"><h2 style="color: #00916d">Lettres permises (@) :</h2><input id="Obligatoireinput"><button id="boutoncoolobligatoire" class="valider">Selection terminée</button></div>';
  ...
  if (!ObligatoiresIndexMaisMieux.includes(txt.length - 1)) {
    ...
    for (let i = 0; i < oldtexte.length; i++) {
      document.getElementById("Obligatoireinput").value +=
        oldtexte[i].toLowerCase();
    }
  }
  ...
  function changestatus() {
    texte = document
      .getElementById("Obligatoireinput")
      .value.toUpperCase()
      .split("");
    ...
    document.getElementById("Obligatoireinput").value =
      "Saisie prise en compte";
    document.getElementById("Obligatoireinput").style =
      "background-color : #11998e; color : white; text-align : center";
...
function SelectInterdit(txt) {
  ...
  divresult.innerHTML +=
    '<div class="Interdits ask"><h2 style="color: #e53935;">Lettres interdites (&) :</h2><input id="Interditinput"><button id="boutoncoolinterdits" class="valider"">Selection terminée</button></div>';
  ...
  function changestatus() {
    ...
    document.getElementById("Interditinput").style =
      "background-color : #11998e; color : white; text-align : center";
...
function detectchanged() {
  ...
  if (document.getElementById("saisie").value.length > max) {
    ...
    document.getElementsByClassName("valider")[0].style =
      "background-color : #e53935; cursor : default";
  } else {
    if (
      document.getElementById("consigne").textContent !=
      "Critère pour le mot cherché :"
    ) {
      document.getElementById("consigne").textContent =
        "Critère pour le mot cherché :";
      document.getElementById("consigne").style = "color : black";
      document.getElementsByClassName("valider")[0].style =
        "background-color : #11998e; cursor : pointer";
    }

    if (
      document.getElementById("saisie").value[
        document.getElementById("saisie").value.length - 1
      ] == "@"
    ) {
...
function go() {
  ...
  function distanceHamming(motatrouver, diff, error) {
    ...
    function SelectObligatoires() {
      ...
      divresult = document.getElementsByClassName("look")[0];
      divresult.innerHTML +=
        '<div class="Obligatoire ask"><h2 style="color: #2c7744">Lettres obligatoires (@) séparés par des espaces</h2><input id="Obligatoireinput"><button id="boutoncoolinterdits" class="valider">Selection terminée</button></div>';
    ...
    for (let caracindex = 0; caracindex < motatrouver.length; caracindex++) {
      if (
        Object.keys(InterditsOuObligatoires).includes(motatrouver[caracindex])
      ) {
      ...
      for (let charac = 0; charac < line.length; charac++) {
        if (line[charac] != motatrouver[charac]) {
          if (Object.keys(special).includes(motatrouver[charac])) {
            ...
            Object.keys(InterditsOuObligatoires).includes(motatrouver[charac])
          ) {
            if (motatrouver[charac] == "&") {
              indexducarac = InterditsIndex.indexOf(charac);
              if (
                InterditsOuObligatoires["&"][indexducarac].includes(
                  line[charac]
                )
              ) {
                score++;
    ...
    if (listefinale.length > 1) {
      ...
      if (error == "none") {
        textresult.innerHTML =
          listefinale.length.toString() + " résultats trouvés";
        textresult.style = "color : #115099;";
      } else {
        textresult.innerHTML =
          "Pas de résultat. Voilà " +
          listefinale.length.toString() +
          " résultats voisins";
      ...
      if (line[charac] != motatrouver[charac]) {
        if (Object.keys(special).includes(motatrouver[charac])) {
          ...
          if (motatrouver[charac] == "&") {
            indexducarac = InterditsIndex.indexOf(charac);
            if (
              InterditsOuObligatoires["&"][indexducarac].includes(line[charac])
            ) {
              score++;
            }
          } else {
...
function generate() {
  ...
  for (let i = 0; i < others.length; i += 1) {
    if (i == 0) {
      others[i].style =
        "margin : 0; margin-top : 50px; border: 5px black solid;border-top: 5px black solid;border-bottom: 2.5px black solid;";
    } else if (i == others.length - 1) {
      others[i].style =
        "margin : 0; border: 5px black solid;border-top: 2.5px black solid;border-bottom: 5px black solid;";
    } else {
      others[i].style =
        "margin : 0; border: 5px black solid;border-top: 2.5px black solid;border-bottom: 2.5px black solid;";
    }
  }
  ...
  if (document.getElementsByClassName("mot2final").length > 0) {
    document
      .getElementsByClassName("mot2final")[0]
      .parentNode.removeChild(document.getElementsByClassName("mot2final")[0]);
  }
  ...
  mot2toreplace.style =
    "display : flex; flex-direction : row; margin : 0px; padding: 0px; border: 0; margin-left: -" +
    margintoto.toString() +
    "vh; width: fit-content;";
...
function create() {
  ...
  if (Mot1Texte[IndexsCaracs[0]] != Mot2Texte[IndexsCaracs[1]]) {
    console.log(
      "Le croisement ne correspond pas ! : " +
        Mot1Texte[IndexsCaracs[0]] +
        " et " +
        Mot2Texte[IndexsCaracs[1]]
    );
    if (
      Mot1Texte[IndexsCaracs[0]] == "*" &&
      (Mot2Texte[IndexsCaracs[1]] == "#" || Mot2Texte[IndexsCaracs[1]] == "/")
    ) {
      Mot1Texte = Mot1Texte.replaceAt(
        IndexsCaracs[0],
        Mot2Texte[IndexsCaracs[1]]
      );
    } else if (
      Mot2Texte[IndexsCaracs[1]] == "*" &&
      (Mot1Texte[IndexsCaracs[0]] == "#" || Mot1Texte[IndexsCaracs[0]] == "/")
    ) {
      Mot2Texte = Mot2Texte.replaceAt(
        IndexsCaracs[1],
        Mot1Texte[IndexsCaracs[0]]
      );
  ...
  if (Mot1Texte[IndexsCaracs[0]] != Mot2Texte[IndexsCaracs[1]]) {
    document.getElementById("whattodo").textContent =
      "Les lettres croisées ne correspondent pas";
    ...
    if (fin.length > 30) {
      ...
      document.getElementsByClassName("mot2final")[0].innerHTML =
        '<h1 style="text-align: center; color : rgb(229, 57, 53);">Il y a trop de combinaisons possibles (' +
        fin.length.toString() +
        ")</h1>";
      document.getElementsByClassName("valider")[0].textContent =
        "Modifier la recherche";
      document
        .getElementsByClassName("valider")[0]
        .setAttribute("onclick", "modifier();");
      ...
      containerRESULTAT =
        document.getElementsByClassName("mot2final")[0].parentNode;
      ...
      document
        ...
        .setAttribute("onchange", "seepreview();");
      ...
      document.getElementsByClassName("mot2final")[0].style = "";
      document.getElementsByClassName("mot2final")[0].innerHTML =
        '<h1 style="text-align: center; color : rgb(229, 57, 53);"> Aucun croisement possible</h1>';
      document.getElementsByClassName("valider")[0].textContent =
        "Modifier la recherche";
      document
        .getElementsByClassName("valider")[0]
...
function modifier() {
  ...
  document.getElementById("whattodo").textContent =
    "Commencez par choisir deux mots";
  document
    .getElementById("buttonvalider1")
    .setAttribute("onclick", "tobutton1()");
  document
    .getElementById("buttonvalider2")
    .setAttribute("onclick", "tobutton2()");
  WordsSelected = 0;

  document.getElementsByClassName("valider")[0].textContent = "Valider";
  document
    .getElementsByClassName("valider")[0]
    .setAttribute("onclick", "create();");
...
function modifytext(lequel) {
  ...
  document.getElementById("buttonvalider" + lequel).style =
    "background-color : #115099";

  if (lequel == "1") {
    document
      .getElementById("buttonvalider" + lequel)
      .setAttribute("onclick", "tobutton1()");
  } else {
    document
      .getElementById("buttonvalider" + lequel)
...
function createit(mot, lequel) {
  ...
  document
    .getElementById("buttonvalider" + lequel)
    .setAttribute("onclick", "modifytext(" + lequel + ")");
  WordsSelected += 1;
  ...
  if (WordsSelected == 2) {
    document.getElementById("whattodo").textContent =
      "Cliquez sur les cases sur lesquelles les mots se croisent";
...
function seepreview() {
  ...
  urlmot2 =
    "https://www.larousse.fr/dictionnaires/francais/" + valeur.split(" ")[1];
  urlmot1 =
    "https://www.larousse.fr/dictionnaires/francais/" + valeur.split(" ")[0];
  ...
  for (let divs = 1; divs < parent.length; divs++) {
    for (let elindex = 1; elindex < parent[divs].childNodes.length; elindex++) {
      if (parent[divs].childNodes[elindex].value == "-") {
        ...
        if (
          matrice[divs - 1][elindex - 1] !=
          parent[divs].childNodes[elindex].value
        ) {
          matrice[divs - 1][elindex - 1] =
            parent[divs].childNodes[elindex].value.toUpperCase();
        }
...
function decaltext(elem, taille) {
  ...
  for (let i = elem.parentNode.childNodes.length - 1; i > 1; i -= 1) {
    elem.parentNode.childNodes[i].value =
      elem.parentNode.childNodes[i - 1].value;
  }
...
function createtable(taille) {
  ...
  var host = document.getElementById("gridHost") || body;
  var tableau = document.createElement("table");
  tableau.id = "tableau";
  tableau.style = "margin-top : 16px;";
  ...
  for (let y = 0; y < TailleY + 1; y++) {
    ...
    if (y == 0) {
      ...
      if (
        window.matchMedia("only screen and (hover: none) and (pointer: coarse)")
          .matches
      ) {
    ...
    for (let x = 0; x < TailleX + 1; x++) {
      if (y == 0) {
        ...
        cell.type = "text";
        cell.style =
          "background-color : #bdbeb6; color: black; border : 0px; border-radius : 0px; overflow:hidden; height : auto; text-align : center; padding : 0px; width : 4vh; padding : 5px; border : 1px solid #bdbeb6; cursor : pointer";
        if (
          window.matchMedia(
        ...
        cell.type = "text";
        cell.style =
          "background-color : #bdbeb6; color: black; border : 0px; border-radius : 0px; overflow:hidden; height : auto; text-align : center; padding : 0px; width : 4vh; cursor : pointer";
        if (
          window.matchMedia(
            "only screen and (hover: none) and (pointer: coarse)"
          ).matches
        ...
        cell.type = "text";
        cell.style =
          "background-color : white; border : 1px solid black; border-radius : 0px; overflow:hidden; height : auto; padding : 5px; width : 4vh; height : 4vh; text-align : center; text-transform: capitalize;";
        if (
          window.matchMedia(
            "only screen and (hover: none) and (pointer: coarse)"
          ).matches
  ...
  host.appendChild(tableau);

  // Scroll into view only if added to body directly
  if (host === body) {
    location.href = "#";
  ...
  divfinal.className = "valider btn primary";

  reseter = document.createElement("button");
  reseter.id = "resetlagrille";
  reseter.textContent = "Réinitialiser la grille";
  reseter.className = "valider btn danger";

  if (host !== body) {
    // Modern layout inside card
    const actions = document.createElement("div");
    actions.className = "grid-actions";
    actions.appendChild(divfinal);
    actions.appendChild(reseter);
    host.appendChild(actions);
  } else {
    // Legacy absolute layout fallback
    divfinal.style = "position: absolute;top: 150%; left: 10%;";
    reseter.style =
      "position: absolute;top: 160%; left: 15px; background-color : #e53935";
  ...
  document
    .getElementById("validerlagrille")
    .setAttribute("onclick", "mainMotcroise4()");

  document
    .getElementById("resetlagrille")
    .setAttribute("onclick", "resettable()");
}
...
document.onkeydown = function (e) {
  ...
  if (page == "croise4mots.html") {
    ...
    function nefocus(nouveauindex) {
      ...
      complicated = false;
      if (diference > 14 || diference < -14) complicated = true;
      ...
      if (complicated) {
        ...
        for (let divs = 0; divs < parent.length; divs++) {
          for (
            let elindex = 0;
            elindex < parent[divs].childNodes.length;
            elindex++
          ) {
    ...
    if (
      (e.keyCode == "106" || e.keyCode == "111" || e.keyCode == "51") &&
      document.activeElement.value != ""
    ) {
      addCaractere(e.key);
    }
    ...
    for (motvertiindex in MotsVerticaux) {
      for (let y = 0; y < MotsVerticaux[motvertiindex][1].length; y++) {
        ...
        if (vraiy == MotsHorrizontaux[motHoriindex][0][1]) {
          ...
          indexmot1 =
            MotsVerticaux[motvertiindex][0][0] -
            MotsHorrizontaux[motHoriindex][0][0] +
            1;

          indexmot2 = vraiy - MotsVerticaux[motvertiindex][0][1] + 1;

          letuple = [
            main(MotsHorrizontaux[motHoriindex][1]),
            indexmot1,
            main(MotsVerticaux[motvertiindex][1]),
            indexmot2,
          ];
...
function mainMotcroise4() {
      ...
      for (let j = 0; j < ListeII.length; j++) {
        ...
        if (listeISTR == listeIISTR) {
          ...
          if (indexcompareI == 0 && indexcompareII == 0) {
            tuplee = [
              ListeI[i][indexcompareI],
              ListeI[i][indexcompareI - 1],
              ListeII[j][indexcompareII - 1],
            ];
  ...
  function communfinal(ListeI, ListeII, indexcompareI, indexcompareII) {
    ...
    for (let i = 0; i < ListeI.length; i++) {
      for (let j = 0; j < ListeII.length; j++) {
        if (
          JSON.stringify(ListeI[i][indexcompareI]) ==
          JSON.stringify(ListeII[j][indexcompareII])
        ) {
          ...
          if (tuplee.length == 4) {
            for (let i = 0; i < tuplee.length; i++) {
              for (let motindex = 0; motindex < tuplee.length; motindex++) {
                if (
                  compteurmots == 1 &&
                  Intersections[0][0][1].includes(tuplee[motindex])
                ) {
                ...
                if (
                  compteurmots == 2 &&
                  Intersections[0][2][1].includes(tuplee[motindex])
                ) {
                  tuplefinalo.push(tuplee[motindex]);
                  compteurmots += 1;
                  break;
                ...
                if (
                  compteurmots == 3 &&
                  Intersections[2][0][1].includes(tuplee[motindex])
                ) {
                  tuplefinalo.push(tuplee[motindex]);
                  compteurmots += 1;
                  break;
                ...
                if (
                  compteurmots == 4 &&
                  Intersections[1][2][1].includes(tuplee[motindex])
                ) {
                  tuplefinalo.push(tuplee[motindex]);
                  compteurmots += 1;
                  break;
  ...
  if (
    ...
    if (final.length == 0) {
      ...
      if (final.length == 1) {
        ...
        textecool.textContent =
          "Il y a " + final.length.toString() + " croisements possibles";
      }
      ...
      document
        .getElementById("nextateur")
        .addEventListener("click", next.bind(null, final, mots));
    }
  } else {
    if (final.length == 0) {
      textecool = document.createElement("h1");
      textecool.id = "letitre";
      textecool.style =
        "position: absolute;top: 140%; right: 5%; color : #115099; font-size : 3vh";
      textecool.textContent = "Il y a aucun croisement possible";
      textecool.style.color = "rgb(229, 57, 53)";
      textecool.style.color.over;
      ...
      textecool2.id = "statuseeee";
      textecool2.style =
        "position: absolute;top: 145%; right: 5%; color : #115099; font-size : 3vh";
      textecool2.textContent = "1 / " + final.length.toString();
...
function previewresultatfinal4mots(liste, initial) {
  ...
  function setmotingrillehori(listedemots, mot) {
    ...
    for (let i = 0; i < listedemots[1].length; i++) {
      inputamodif =
        tableau[listedemots[0][1] + 1].childNodes[listedemots[0][0] + 1 + i];
  ...
  function setmotingrilleverti(listedemots, mot) {
    ...
    for (let i = 0; i < listedemots[1].length; i++) {
      inputamodif =
        tableau[listedemots[0][1] + 1 + i].childNodes[listedemots[0][0] + 1];
...
function next(final, mots) {
  ...
  document.getElementById("statuseeee").textContent =
    (indexlistealire + 1).toString() + " / " + final.length.toString();
  previewresultatfinal4mots(final[indexlistealire], mots);
}
